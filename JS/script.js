...
d3.csv("../data/Most Streamed Spotify Songs 2024.csv", (d) => {
...
function renderScene(sceneIndex) {
  ...
  if (sceneIndex === 0) {
    ...
    // Optional: Intro label
    container.append("p")
      .attr("class", "intro-label")
      .text("Introduction");

    // Add the actual takeaway text
    container.append("p")
  ...
  const annotations = [
    {
      note: {
        ...
        wrap: 150,
        align: "middle"  // alignment inside the note itself
      },
...
function scene2() {
  ...
  svg.selectAll(".top-platform-label")
    ...
    .attr("x", d => x(d.date))
    .attr("y", height + 25) // position below x axis
    .attr("text-anchor", "middle")
    .attr("font-size", "10px")
    .attr("fill", "#DFAC73")
